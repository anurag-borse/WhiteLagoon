...
namespace WhiteLagoon.Application.Services.Implementation
{
    public class BookingService : IBookingService
    {
        ...
        public IEnumerable<Booking> GetAllBookings(string userId = "", string? statusFilteredList = "")
        {
            IEnumerable<string> statusList = statusFilteredList.ToLower().Split(",");

            if (!string.IsNullOrEmpty(statusFilteredList) && !string.IsNullOrEmpty(userId))
            {
                return _unitOfWork.Booking.GetAll(u => statusList.Contains(u.Status.ToLower()) && u.UserId == userId, includeProperties: "User,Villa");
            }
            else
            {
                if (!string.IsNullOrEmpty(statusFilteredList))
                {
